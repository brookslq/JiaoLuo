...
class RealmManager: NSObject {
    // MARK: - 单例实例
    static var `default`: RealmManager = RealmManager()
    ...
    ///   - isReadOnly: 是否是只读
    /// - Returns: Realm实例
    public func createDB(_ dataBaseName: String, isReadOnly: Bool = false) -> Realm? {
    ...
    /// - isReadOnly: 是否是只读
    /// - Returns: Realm实例
    func openDB(_ dataBaseName: String, isReadOnly: Bool = false) -> Realm? {
        guard let dbPath = getCreateDatabasePath(dataBaseName) else {
            return nil
        }
        // 数据库配置
    ...
    public func setDefaltRealmConfiguration(_ realmName: String,isReference: Bool = false, isReadOnly: Bool = false) -> Bool{
        var realmPath: URL?
        if isReference {
            realmPath = getReferenceDatabasePath(realmName)
        }else {
            realmPath = getCreateDatabasePath(realmName)
        }
        if realmPath == nil {
            return false
        }
    ...
    public func openReferenceDB(_ dataBaseName: String, isReadOnly: Bool = true) -> Realm? {
        guard let dbPath = getReferenceDatabasePath(dataBaseName) else {
            return nil
        }
        var config = Realm.Configuration()
        config.fileURL = dbPath
    ...
    public func updateObject( property: inout Any, value: Any) -> Bool {
        ...
        do {
            ...
            mLog(message: "直接更新对象属性错误: \(error.localizedDescription)")
            return false
        }
    }
    ...
    public func updateObjects(type: RealmSwift.Object.Type, key: String, value: Any) -> Bool {
        ...
        do {
            ...
            mLog(message: "更改一个表中的所有数据错误: \(error.localizedDescription)")
            return false
        }
    }
    ...
    public func updateObject(type: RealmSwift.Object.Type, primaryKey: Any, key: String, value: Any) -> Bool {
        ...
        do {
            ...
            mLog(message: "更新数据出错: \(error.localizedDescription)")
            return false
        }
    }
...
extension RealmManager {
    ...
    /// - Returns: 数据库保存的路径
    private func getCreateDatabasePath(_ fileName: String) -> URL? {
        let cachesPath = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first
        guard var filePath = cachesPath else {
            return nil
        }
        filePath = filePath + "/RealmDB/\(fileName)"
        ...
        do {
            ...
            mLog(message: "创建数据库文件夹失败：\(error.localizedDescription)")
            return nil
        }
    ...
    /// - 参数fileName：数据库的名字
    /// - 返回：本地引用的数据库的路径
    private func getReferenceDatabasePath (_ fileName: String) -> URL? {
        let path = Bundle.main.path(forResource: fileName, ofType: "realm")
        let url = path != nil ? URL(string: path!) : nil
        return url
    }
}
