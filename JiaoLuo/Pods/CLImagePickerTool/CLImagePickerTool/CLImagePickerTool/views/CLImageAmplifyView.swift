...
//        self.scrollView?.frame = (UIApplication.shared.keyWindow?.bounds)!
        let viewH: CGFloat = UIDevice.current.isX() == true ? 44+34:44
        let cutH = UIDevice.current.isX() ? 44 + viewH : 24 + viewH
        self.scrollView?.frame = CGRect(x: 0, y: UIDevice.current.isX() ? 44 : 24, width: KScreenWidth, height: KScreenHeight - cutH)
